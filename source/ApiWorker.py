...
class ApiWorker:
    ...
    def get_last_posts(self):
        try:
            posts = self.vk_api.wall.get(owner_id=-33676586, count=20)
            return posts
        except:
    ...
    def before_post(self):
        ...
        if data[1] != '' and data[1] != None:
            ...
            temp_photo = requests.post(upload_server['upload_url'], files={'photo': open('source/tmp/result.png', 'rb')}).json()
            #'photo': b['photo'], 'server': b['server'], 'hash': b['hash']})[0]d =
            save_method = self.vk_api.photos.saveWallPhoto(group_id=99558704,photo=temp_photo['photo'],server=temp_photo['server'], hash=temp_photo['hash'])[0]
